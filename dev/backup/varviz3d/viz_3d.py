# -*- coding: utf-8 -*-
"""
VarViz3D backend — data API + 3D viewer static hosting

Run:
    python backend.py
Then open (for a quick check):
    http://localhost:5001/3d/viewer?uniprot=P04637&win=15&class=any&highlight=175
"""
...
STATIC_3D_DIR = os.path.join(os.path.dirname(__file__), "static", "3d")

# ----- Optional local modules -----
try:
    from gene_to_uniprot import UniProtResolver  # optional resolver
except Exception:
...
try:
    import viz_3d as viz
except Exception as e:
    raise RuntimeError(f"viz_3d.py must be present next to backend.py: {e}")
